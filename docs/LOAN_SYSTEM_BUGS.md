...
Status verified 2026-06-14 against current code.

| # | Severity | Component | Description | Status (2026-06-14) |
|---|----------|-----------|-------------|---------------------|
| 1 | CRITICAL | Repayment Upload | Wrong/missing Excel columns → mass penalty on all borrowers | **FIXED** — header validation still runs before payroll AWAITING rows are generated, and the fail-sweep is no longer triggered by upload jobs at all; it now runs only from the explicit close-period action |
...
| 8 | MAJOR | Config Counters | BALANCE_OUTSTANDING doesn't grow with penalties, drifts negative over time | **FIXED** — the explicit close-period job now tops up `BALANCE_OUTSTANDING` by the total penalty added when remaining payroll AWAITING rows are failed |
...
| 10 | MINOR | Repayment Upload | Re-uploading same period creates duplicate MANUAL_RESOLUTION records | **FIXED / REWORKED** — exact duplicate files are blocked by `RepaymentUpload.fileHash`, re-saved files are blocked per staff by skipping rows once a non-`AWAITING` `PAYROLL` repayment exists for that user+period, and `LAST_REPAYMENT_DATE` now closes a period explicitly instead of blocking all same-period uploads |
...
| 15 | MINOR | Schedule Report | Tenure shown is span from oldest loan start to newest loan end, not actual tenure | **FIXED / CHANGED** — tenure now sums each loan's `tenure + extension` instead of `differenceInMonths(end, firstStart)` (`queue.reports.ts:154`). Sum-of-tenures semantics are still debatable for multi-loan customers |
